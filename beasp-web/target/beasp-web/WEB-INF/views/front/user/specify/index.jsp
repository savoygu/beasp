...
<title>${spuser.nickName }的个人空间-书籍交换与分享平台</title>
	...
	<section id="main">
		<div class="main-body w-1200 container cf">
			...
			<div class="r space-main">
				<div class="family cf">
					<h1 class="l family-hd">TA的书籍</h1>
				</div>
				<!-- 工具条开始 -->
				<div class="book-tool-bar cf">
					<div class="tool-left l">
						<a class="sort-item <c:if test='${empty status or status < 1 or status > 3 }'>active</c:if>" href="<%=beaspPath %>/space/u/uid/${id}">待审核</a>
						<a class="sort-item <c:if test='${status eq 3 }'>active</c:if>" href="<%=beaspPath %>/space/u/uid/${id}/book/s/3">发布</a>
						<a class="sort-item <c:if test='${status eq 2 }'>active</c:if>" href="<%=beaspPath %>/space/u/uid/${id}/book/s/2">分享</a>
						<a class="sort-item <c:if test='${status eq 1 }'>active</c:if>" href="<%=beaspPath %>/space/u/uid/${id}/book/s/1">交换</a>
					</div>
