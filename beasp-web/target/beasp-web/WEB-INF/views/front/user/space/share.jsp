...
<title>${user.nickName }的分享-书籍交换与分享平台</title>
	...
	<section id="main">
		<div class="main-body w-1200 container cf">
			...
			<div class="r space-main">
				<div class="family cf">
					<h1 class="l family-hd">我的分享</h1>
				</div>
				<!-- 工具条开始 -->
				<div class="book-tool-bar cf">
					<div class="tool-left l">
						<a class="sort-item <c:if test='${empty way or way < 1 or way > 1 }'>active</c:if>" href="<%=beaspPath %>/space/share">TA人分享</a>
						<a class="sort-item <c:if test='${way eq 1 }'>active</c:if>" href="<%=beaspPath %>/space/share/w/1">分享TA人</a>
					</div>
						...
						<div class="tool-all">
							...
							<ul id="js-columnbd" class="all-cont" style="margin-left: -59px;">
								<c:if test='${empty way or way < 1 or way > 1 }'>
									<li><a href="<%=beaspPath%>/space/share/cid/0" data-id="0">全部</a></li>
									<c:forEach items="${categories }" var="c">
										<li><a href="<%=beaspPath%>/space/share/cid/${c.id}" data-id="${c.id }" >${c.name }</a></li>
									</c:forEach>
								</c:if>
								<c:if test="${!empty way and way eq 1 }">
									<li><a href="<%=beaspPath%>/space/share/w/${way}/cid/0" data-id="0">全部</a></li>
									<c:forEach items="${categories }" var="c">
										<li><a href="<%=beaspPath%>/space/share/w/${way}/cid/${c.id}" data-id="${c.id }">${c.name }</a></li>
									</c:forEach>
								</c:if>
							</ul>
						</div>
				...
				<c:if test="${empty page or page.numberOfElements eq 0}">
					<ul class="follow-list">
			          <div class="unbook">
				        <p>暂无任何<c:if test="${way lt 1 or way gt 1 or empty way }">TA人分享</c:if><c:if test="${way eq 1 }">分享TA人</c:if>书籍</p>
				      </div>
			        </ul>
				</c:if>
											...
											<ul class="cf">
												<c:forEach items="${page.content }" var="share">
													<c:set var="createTime">
														<fmt:formatDate value="${share.applyTime }" pattern="yyyy-MM-dd"/>
													</c:set>
													<c:if test="${createTime eq curTime}">
														<li class="book-one" data-id="${share.target.id }">
															<a href="<%=beaspPath%>/book/view/${share.target.id}">
																<div class="book-state">
																	<h2>${share.state.name }</h2>
																</div>
																<div class="book-list-img">
																	<c:forEach items="${share.target.styles }" var="style">
																		<c:if test="${style.choice eq true }">
																			<img alt="${share.target.name }" src="<%=beaspPath %>${style.imageFullPath}" height="135" width="240">
																		</c:if>
																	</c:forEach>
																	...
																	<c:if test="${fn:length(collections) != 0 }">
																		<i class="btn-add-collection <c:if test='${!empty collection_book[share.target.id]}'>btn-remove-collection </c:if>"></i>
																...
																<h5><span>${share.target.name }</span></h5>
																<div class="tips">
																	<p class="text-ellipsis">${share.target.summary }</p>
																	<span class="l"><i class="fa fa-list-ul"></i> ${share.target.category.name }</span>
																	<span class="l mar-l20"><i class="fa fa-eye"></i> ${share.target.browse }</span>
																</div>
																<span class="time-label">${share.target.author }</span>
															</a>
															
															<c:if test="${way lt 1 or way gt 1 or empty way }">
																<c:if test="${share.state eq 'SHAREING' }">
																	<div class="book-oper" style="display: none;">
																		<%-- <a href="<%=beaspPath%>/user/share/${share.target.id}/aggre" >同意分享</a>
																		<a href="<%=beaspPath%>/user/share/${share.target.id}/disaggre">不同意</a> --%>
																		<a href="javascript:void(0)" data-bookId="${share.target.id }" id="js-aggre-share">同意分享</a>
																		<a href="javascript:void(0)" data-bookId="${share.target.id }" id="js-disaggre-share">不同意</a>
																	</div>
																</c:if>
															</c:if>
														</li>
